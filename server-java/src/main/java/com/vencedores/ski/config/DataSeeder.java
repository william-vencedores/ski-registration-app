...
public class DataSeeder implements CommandLineRunner {
    ...
    private void seedDefaultDisclosures() {
        ...
        if (!existing.isEmpty()) {
            log.info("Disclosures already exist ({}), skipping seed", existing.size());
            return;
        }
        ...
        log.info("Created liability waiver disclosure: {}", liability.get("id"));
        ...
        log.info("Created medical authorization disclosure: {}", medical.get("id"));

        // Attach both disclosures to all active events
        var events = eventService.listEvents(false);
        for (var event : events) {
            String eventId = (String) event.get("id");
            disclosureService.attachToEvent(eventId, (String) liability.get("id"), 1);
            disclosureService.attachToEvent(eventId, (String) medical.get("id"), 2);
