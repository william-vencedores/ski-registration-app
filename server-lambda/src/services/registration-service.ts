...
function buildMinorItem(
  ...
  const item: Record<string, unknown> = {
    ...
    skillLevel: '',
    dietary: '',
    // Medical info collected per minor; defaults keep older clients (which only
    // sent name + dob) working.
    medConditions: minor.medConditions ?? 'no',
    conditionDetails: minor.conditionDetails ?? '',
    medAllergies: minor.medAllergies ?? 'no',
    allergyDetails: minor.allergyDetails ?? '',
    medMedications: minor.medMedications ?? 'no',
    medicationDetails: minor.medicationDetails ?? '',
